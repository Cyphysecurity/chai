...
class DetectorVlmPipeline(WrapperBase):
    def __init__(
        ...
        self.overscan_value = 0  # pixels in each direction
    ...
    def run_inference(
        ...
        if filter_results:
            ...
            masks, boxes_filt, scores, _ = to_filter

            if len(masks) == 0:
                logger.info("No matches found.")
                return image_pil, None, None, None, justifications
...
def get_vlm(vl_model, system_description, simulate_time_delay, device):
    if "gpt" in vl_model:
...
def get_vlm_pipeline(
    ...
    """
    # models
    vlm = get_vlm(vl_model_name, system_description, simulate_time_delay, device)
